...
3. Run `spastic vault setup`
4. Run `spastic deploy` to attach the vault to sessions
...
3. Name it (e.g., "spastic"), select your workspace
...
2. Add **User Token Scopes** (NOT Bot Token Scopes). The Slack MCP server needs read access to every surface the bot might be invoked from (DMs, private channels, group DMs) plus identity to map users:
   ...
   Omitting any of these surfaces will produce a 400 at MCP `initialize` with a `missing_scope` body. The strict subset the docs used to ship (`search:read,channels:read,channels:history,chat:write,files:read`) does not work — it produces a 400 because `users:read` is missing for identity lookup.
...
See the `mcp-proxy` project in protohype (or scaffold one with `spastic scaffold`).
...
1. Go to https://console.cloud.google.com → create project (e.g., "spastic")
