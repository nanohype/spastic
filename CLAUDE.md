...
- **Memory is an MCP server**: company memory is no longer file-based. Agents call `memory_query` / `memory_store` / `memory_list` / `memory_delete` on the gateway's `/memory` endpoint. Each agent's `agentId` is its role (or `"coordinator"`).
