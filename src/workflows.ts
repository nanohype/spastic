...
export const WORKFLOWS: Workflow[] = [
  {
    ...
    steps: [
      ...
      { role: 'tech-writer', instruction: 'Write API docs, user guides, and changelog for the launch.' },
    ],
  },
  {
    name: 'feature-build',
    ...
    steps: [
      ...
      { role: 'design', instruction: 'Define the UI components, tokens, and interaction patterns for this feature.' },
      {
        role: 'eng-frontend',
  ...
  },
  {
    ...
    steps: [
      ...
      { role: 'eng-backend', instruction: 'Diagnose root cause, implement a fix, and describe what changed.' },
      { role: 'qa-automation', instruction: 'Define regression tests to prevent recurrence and validate the fix.' },
      {
        role: 'ops-incident',
  ...
  },
  {
    ...
    steps: [
      ...
      { role: 'sales', instruction: 'Build battle cards and proposal templates aligned with the campaign messaging.' },
    ],
  },
  {
    name: 'lead-gen',
  ...
  },
  {
    ...
    steps: [
      ...
      { role: 'legal', instruction: 'Review and customize the service agreement. Flag any non-standard terms.' },
      { role: 'customer-success', instruction: 'Prepare the onboarding plan so handoff is immediate after signature.' },
    ],
  },
  {
    name: 'content-engine',
    ...
    steps: [
      ...
      { role: 'marketing-email', instruction: 'Design email sequences to distribute the content to segmented lists.' },
      {
        role: 'data-analyst',
  ...
  },
  {
    ...
    steps: [
      ...
      { role: 'legal', instruction: 'Review privacy policy and DPA alignment with the security audit findings.' },
    ],
  },
  {
    name: 'perf-review',
  ...
  },
  {
    ...
    steps: [
      ...
      { role: 'engineering', instruction: 'Estimate capacity and flag technical risks for the prioritized items.' },
      {
        role: 'design',
  ...
  },
  {
    ...
    steps: [
      ...
      { role: 'sales', instruction: 'Prepare expansion proposal based on usage data and identified opportunities.' },
      {
        role: 'product-growth',
        instruction:
  ...
  },
  {
    ...
    steps: [
      ...
      { role: 'qa', instruction: 'Review data quality findings and prioritize fixes by business impact.' },
      {
        role: 'data-analyst',
        instruction: 'Verify dashboard accuracy against source data. Flag stale or misleading metrics.',
      },
      { role: 'operations', instruction: 'Update operational procedures for data pipeline monitoring and alerting.' },
    ],
  },
];
...
export async function executeWorkflow(
  ...
  if (workflow.gateProfile === 'code') {
    ...
    if (!branchContext) {
      ...
      console.log(`${DIM}If no primary repo is configured: spastic repo add <github-url> --token <github-pat>${RESET}`);
      return;
    }
...
${step.instruction}`,
  ...
  if (workflow.gateProfile) {
    const gateResult = await runMergeGate(api, workflow.name, workflow.gateProfile, context);
...
Review the PR candidate against your role's merge-gate criteria per FACTORY_PREAMBLE. End your response with the full block: GATE_VERDICT, GATE_FEEDBACK, TRANSCRIPTS, CITATIONS, QUALITY_GRADES — EVIDENCE_CONTRACT auto-downgrades APPROVE/REQUEST_CHANGES without transcripts + citations to REJECT.`,
    ...
    if (lastResult.decision === 'approve') {
      ...
      if (profile === 'code') {
        const calibration = await runExternalCalibration(api, workflowName, verdicts, context);
...
5. After revisions complete, produce an updated artifact manifest`;
  ...
  await sendAndStream(api, sessionId, message);
}

// ── Helpers ─────────────────────────────────────────────────────────

async function sendAndStream(api: AnthropicAgents, sessionId: string, message: string): Promise<string> {
  await api.sendMessage(sessionId, message);
  return streamWithAdvisor(api, sessionId);
}
...
async function runRoleSession(
  ...
  if (!entry) {
    throw new Error(`Role "${role}" is not deployed. Run: spastic deploy`);
  }
  const envId = await getEnvironmentId();
  if (!envId) {
    throw new Error(`No environment configured. Run: spastic deploy`);
  }
  ...
  await api.sendMessage(sess.id, message);
  return streamWithAdvisor(api, sess.id, {
...
export async function streamWithAdvisor(
  ...
  for await (const event of api.stream(sessionId)) {
        ...
        for (const eventId of stopReason.event_ids) {
          ...
          if (pending?.kind === 'custom' && pending.name === 'consult_advisor') {
            ...
            if (advisorCalls >= maxAdvisorCalls) {
              ...
              try {
                await api.sendCustomToolResult(
                  sessionId,
                  eventId,
                  `Advisor budget exhausted for this session (${advisorCalls}/${maxAdvisorCalls} calls used). Make the decision with the context you have and document your reasoning.`,
                  true,
                );
              } catch {
                /* best effort */
            ...
            try {
              ...
              await api.sendCustomToolResult(sessionId, eventId, advice);
            } catch (err) {
              try {
                await api.sendCustomToolResult(
                  sessionId,
                  eventId,
                  `Advisor unavailable: ${err instanceof Error ? err.message : String(err)}`,
                  true,
                );
              } catch (sendErr) {
                process.stdout.write(
          ...
          if (pending?.kind === 'builtin') {
            ...
            try {
              await api.confirmTool(sessionId, eventId, decision);
            } catch (confirmErr) {
              process.stdout.write(
