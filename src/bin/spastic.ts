...
async function deploy(args: ParsedArgs): Promise<void> {
  ...
  if (!skipSkills) {
    ...
    for (const [role, def] of getAllSkillDefs()) {
      const { content, referenceFiles } = await loadSkillContent(role, nanohypePath, TEAM);
      const skillPayload = {
        name: def.name,
  ...
  // Coordinator — create or update
  const coord = TEAM.find((m) => m.role === 'coordinator')!;
  const { servers, tools } = resolveMcpServers(coord.mcpServers);
  const coordSkills: SkillReference[] = [];
  if (skillRefs['coordinator']) coordSkills.push(skillRefs['coordinator']);

  if (dryRun) {
    const existing = state.agents.find((a) => a.role === 'coordinator');
    const placeholderCallable = members.map((m) => {
      const e = state.agents.find((a) => a.role === m.role);
      return { type: 'agent' as const, id: e?.agentId ?? `<${m.role}-agent-id>`, version: e?.version ?? 1 };
    });
    console.log(`coordinator:${existing ? ` (update ${existing.agentId} v${existing.version})` : ' (create)'}`);
    const coordModelId = overrides['coordinator'] ?? coord.model;
    console.log(
      JSON.stringify(
        {
          name: coord.name,
          model: fastMode ? { id: coordModelId, speed: 'fast' } : coordModelId,
          description: coord.description,
          system: buildSystemPrompt(coord, state),
          mcp_servers: servers,
          tools: [
            {
              type: 'agent_toolset_20260401',
            ...
            ...tools,
            ...advisorToolsFor('coordinator'),
          ],
          ...(coordSkills.length > 0 && { skills: coordSkills }),
          callable_agents: placeholderCallable,
          metadata: { spastic_role: 'coordinator' },
        },
        null,
        2,
      ),
    ...
    const callableAgents = state.agents
      .filter((a) => a.role !== 'coordinator')
      .map((a) => ({ type: 'agent' as const, id: a.agentId, version: a.version }));

    const realCoordModelId = overrides['coordinator'] ?? coord.model;
    const coordParams: AgentCreateParams = {
      name: coord.name,
      model: fastMode ? { id: realCoordModelId, speed: 'fast' } : realCoordModelId,
      description: coord.description,
      system: buildSystemPrompt(coord, state),
      mcp_servers: servers,
      tools: [
    ...
    const coordAgent = await deployAgent(api!, state, 'coordinator', coordParams);
    state.agents = state.agents.filter((a) => a.role !== 'coordinator');
    state.agents.push({
      role: 'coordinator',
      agentId: coordAgent.id,
      version: coordAgent.version,
      deployedAt: coordAgent.updated_at ?? coordAgent.created_at,
    });
    state.coordinatorId = coordAgent.id;
    ...
    console.log(`Coordinator can delegate to ${callableAgents.length} agents`);
...
async function status(): Promise<void> {
  ...
  if (state.coordinatorId) {
    console.log(`\nCoordinator: ${state.coordinatorId}`);
  }
}
...
async function session(args: ParsedArgs): Promise<void> {
  ...
  if (!roleName) {
    ...
    console.error(
      'Roles: coordinator, product, design, engineering, qa, sales, marketing, operations, customer-success',
    );
    process.exit(1);
  }
...
async function adopt(args: ParsedArgs): Promise<void> {
  ...
  const role = (args.positional[0] ?? 'coordinator') as TeamRole;

  if (!agentId) {
    console.error('Usage: spastic adopt <agent-id> [role]');
    console.error('Default role: coordinator');
    process.exit(1);
  }
  ...
  if (role === 'coordinator') state.coordinatorId = agent.id;
  await saveState(state);
...
async function standup(args: ParsedArgs): Promise<void> {
  ...
  // Route through coordinator if --session provided, otherwise use coordinator's agent
  let sessionId = typeof args.flags.session === 'string' ? args.flags.session : undefined;

  if (!sessionId) {
    const entry = await getAgentByRole('coordinator');
    if (!entry) {
      console.error('No deployed coordinator. Run: spastic deploy');
      process.exit(1);
      return;
    }
...
async function workflow(args: ParsedArgs): Promise<void> {
  ...
  // Use existing coordinator session or create one
  let sessionId = typeof args.flags.session === 'string' ? args.flags.session : undefined;
  ...
  if (!sessionId) {
    ...
    const sess = await createSession(api, entry.agentId, `workflow: ${name}`);
    sessionId = sess.id;
  }
  ...
  if (!skipIntake) {
    ...
    if (intakeEntry) {
      ...
      await api.sendMessage(
        intakeSess.id,
        `Validate and enrich this intake for the "${name}" workflow. Return the validated intake as a structured block the coordinator can parse directly.\n\nINTAKE:\n${prompt}`,
      );
...
async function skillsShow(args: ParsedArgs): Promise<void> {
  ...
  const content = await previewSkillContent(role, nanohypePath, TEAM);
  console.log(content);
}
...
USAGE
  ...
  spastic standup [--session <id>]      Run a team standup via coordinator
...
ROLES
  coordinator, product, design, engineering, qa,
  sales, marketing, operations, customer-success
...
EXAMPLES
  ...
  spastic session coordinator --title "Q2 planning"
...
async function recover(): Promise<void> {
  ...
  const coord = matched.find((a) => a.role === 'coordinator');
  state.coordinatorId = coord?.agentId ?? null;
